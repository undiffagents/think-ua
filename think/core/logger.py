...
def get_think_logger(name="think", logfilename="outfile.txt", uselogfile=False, formats="%(time)12.3f      %(source)-18s      %(message)s", level=logging.DEBUG):
        """Returns a new logger."""
        if uselogfile:
            logging.basicConfig(filename=logfilename,format=formats, level=level)
        else:
